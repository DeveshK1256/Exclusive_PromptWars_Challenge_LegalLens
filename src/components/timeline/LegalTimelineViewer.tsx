  ...
  return (
    <div className="space-y-6" role="region" aria-label="Legal Document Timeline">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <div className="bg-slate-950 px-3.5 py-2 rounded-lg border border-slate-800 text-xs">
            ...
            <span className="font-bold text-indigo-400">{events.length}</span>
          </div>
          {upcomingDeadlinesCount > 0 && (
            <div className="bg-amber-500/10 border border-amber-500/20 px-3.5 py-2 rounded-lg text-xs flex items-center gap-1.5 text-amber-400 font-semibold" role="status">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 shadow-sm">
        {events.length === 0 ? (
          <p className="text-xs text-slate-400 text-center py-8">No specific calendar dates or deadlines were detected in this document.</p>
        ) : (
          <div className="relative pl-6 space-y-8 before:absolute before:left-2.5 before:top-2 before:bottom-2 before:w-0.5 before:bg-slate-800" role="list">
            {events.map((evt, idx) => {
              ...
              return (
                <div key={evt.id || idx} className="relative group" role="listitem">
                  {/* Timeline Dot */}
                  <div className="absolute -left-6 top-1.5 w-4 h-4 rounded-full bg-slate-900 border-2 border-indigo-500 group-hover:border-indigo-400 group-hover:scale-110 transition-all flex items-center justify-center" aria-hidden="true">
                    <div className="w-1.5 h-1.5 rounded-full bg-indigo-400" />
                  </div>

                  {/* Event Card */}
                  <div className="bg-slate-950/80 border border-slate-800/80 hover:border-slate-750 p-4 rounded-xl space-y-2.5 transition-all shadow-sm">
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      ...
                      {evt.event_date && (
                        <div className="text-xs font-mono text-indigo-300 font-semibold bg-indigo-500/10 px-2.5 py-1 rounded border border-indigo-500/20 flex items-center gap-1.5">
