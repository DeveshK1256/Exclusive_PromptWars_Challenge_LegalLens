...
export const DocumentQAChat: React.FC<DocumentQAChatProps> = ({
  ...
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-xl flex flex-col h-[650px] shadow-sm">
      {/* Header */}
      <div className="p-4 border-b border-slate-800 bg-slate-950/60 rounded-t-xl flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400">Ask questions directly grounded in {documentTitle}</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-5">
        {messages.length === 0 ? (
          <div className="text-center py-10 px-4 max-w-md mx-auto space-y-4">
            <div className="w-12 h-12 rounded-full bg-slate-800/80 flex items-center justify-center mx-auto text-indigo-400">
              <Sparkles className="w-6 h-6" />
            </div>
            <div>
              <h4 className="text-sm font-semibold text-slate-200">Ask anything about this document</h4>
            ...
            <div className="pt-2 text-left space-y-2">
              ...
              {sampleQuestions.map((sq, i) => (
                <button
                  ...
                  className="w-full text-left p-2.5 text-xs text-slate-300 bg-slate-950/60 hover:bg-slate-800/80 rounded-lg border border-slate-800 transition-colors flex items-center justify-between group"
                >
                  <span>{sq}</span>
                  <HelpCircle className="w-3.5 h-3.5 text-slate-500 group-hover:text-indigo-400 shrink-0" />
                </button>
              ))}
            </div>
          </div>
          ...
          messages.map((msg, i) => (
            <div key={i} className="space-y-3">
              ...
              <div className="flex justify-start">
                <div className="bg-slate-950 border border-slate-800 text-slate-200 text-xs p-4 rounded-2xl rounded-tl-none max-w-xl space-y-3 shadow-sm">
                  ...
                  {/* Answer Text */}
                  <p className="leading-relaxed whitespace-pre-line text-slate-300">{msg.answerText}</p>
                    ...
                    <div className="pt-2 space-y-2 border-t border-slate-800/60">
                      ...
                      <div className="space-y-1.5">
                        {msg.citations.map((c, cIdx) => (
                          <div
                            key={cIdx}
                            className="bg-slate-900/80 p-2.5 rounded-lg border border-slate-800 text-[11px] text-slate-300 space-y-1"
                          >
                            <div className="flex items-center justify-between text-indigo-400 font-mono text-[10px]">
                              <span>{c.sectionTitle}</span>
                              <span className="bg-indigo-500/10 px-1.5 py-0.5 rounded border border-indigo-500/20">
                                {c.sourceReference} (Score: {(c.relevanceScore * 100).toFixed(0)}%)
                              </span>
                            </div>
                            <p className="text-slate-400 italic font-mono text-[10px] line-clamp-2">"{c.quotedTextSnippet}"</p>
                          </div>
                        ))}
                      </div>
                    </div>
                    ...
                    <div className="pt-2 border-t border-slate-800/60 flex flex-wrap gap-1.5">
                      {msg.suggestedFollowUpQuestions.map((fu, fIdx) => (
                        <button
                          ...
                          className="text-[10px] bg-slate-900 hover:bg-slate-800 text-slate-300 px-2.5 py-1 rounded-full border border-slate-750 transition-colors"
                        >
                          + {fu}
                        </button>
                      ))}
        ...
        {loading && (
          <div className="flex justify-start">
            <div className="bg-slate-950 border border-slate-800 text-xs px-4 py-3 rounded-2xl rounded-tl-none text-slate-400 flex items-center gap-2">
      ...
      <div className="p-3 border-t border-slate-800 bg-slate-950/60 rounded-b-xl">
        ...
        >
          <input
            ...
            disabled={loading}
            className="flex-1 bg-slate-900 border border-slate-800 text-xs text-slate-200 placeholder-slate-500 rounded-lg px-3.5 py-2.5 focus:outline-none focus:border-indigo-500 transition-colors"
          />
          <button
            type="submit"
            disabled={loading || !questionInput.trim()}
            className="bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white p-2.5 rounded-lg transition-colors flex items-center justify-center shrink-0"
          >
