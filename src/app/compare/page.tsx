      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 space-y-3">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold uppercase tracking-wider text-indigo-400">Document Version A (Base)</span>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 flex flex-col md:flex-row md:items-center justify-between gap-6">
            ...
            <div className="flex items-center gap-4">
              <div className="bg-slate-950 px-4 py-2 rounded-lg border border-slate-800 text-center">
                <span className="text-xs text-slate-400 block">Identified Differences</span>
                <span className="text-lg font-extrabold text-amber-400">{comparisonResult.differencesCount}</span>
              </div>
              <div className="bg-slate-950 px-4 py-2 rounded-lg border border-slate-800 text-center">
                <span className="text-xs text-slate-400 block">Matching Provisions</span>
                <span className="text-lg font-extrabold text-emerald-400">{comparisonResult.similaritiesCount}</span>
              </div>
            </div>
          </div>
              ...
              {comparisonResult.findings.map((f, idx) => (
                ...
                >
                  <div className="flex items-center justify-between flex-wrap gap-2">
                    ...
                    <span className="text-xs text-slate-400 font-mono">Category: {f.category}</span>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
                    <div className="bg-slate-900/80 p-3 rounded-lg border border-slate-800">
                      <span className="text-indigo-400 font-semibold block mb-1">Doc A ({docATitle}):</span>
                      <p className="text-slate-200">{f.document_a_value}</p>
                    ...
                    <div className="bg-slate-900/80 p-3 rounded-lg border border-purple-500/30">
                      <span className="text-purple-300 font-semibold block mb-1">Doc B ({docBTitle}):</span>
                      <p className="text-slate-200">{f.document_b_value}</p>
                  ...
                  <p className="text-xs text-slate-300 bg-slate-900 p-3 rounded-lg border border-slate-800">
                    <strong className="text-indigo-400">Difference Analysis: </strong> {f.difference_summary}
                  </p>
                </div>
              ))}
            </div>
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 space-y-4">
              ...
              <ul className="space-y-2 text-xs text-slate-300">
                {comparisonResult.questionsForLawyer.map((q, idx) => (
                  <li key={idx} className="bg-slate-950 p-3 rounded-lg border border-slate-800 flex items-start gap-2">
