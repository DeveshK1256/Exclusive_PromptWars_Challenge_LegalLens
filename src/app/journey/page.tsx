    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      <div className="border-b border-slate-800 pb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-3 flex items-center gap-3">
          <label htmlFor="context-perspective-select" className="text-xs font-bold text-slate-300">Perspective Context:</label>
          <select
            ...
            className="bg-slate-950 border border-slate-800 text-indigo-300 text-xs font-semibold rounded-lg px-3 py-1.5 focus:outline-none focus:ring-2 focus:ring-indigo-500"
          >
      ...
      {impactResult?.degraded && (
        <div className="bg-amber-500/10 border border-amber-500/40 rounded-xl p-4 text-amber-200 text-xs flex items-center gap-3" role="alert" aria-live="assertive">
          <AlertTriangle className="w-5 h-5 text-amber-400 shrink-0" aria-hidden="true" />
          <div>
            <strong className="block font-bold text-amber-300">AI Personal Impact Temporarily Offline (Degraded Mode)</strong>
          ...
          return (
            <button
              ...
              className={`p-4 rounded-xl border flex items-center gap-3 transition text-left focus:outline-none focus:ring-2 focus:ring-indigo-500 ${
                isActive
                  ? 'bg-indigo-600/10 border-indigo-500/50 text-slate-100 shadow-md'
                  : isDone
            ...
            >
              <div
                className={`p-2 rounded-lg ${
                  isActive ? 'bg-indigo-600 text-white' : isDone ? 'bg-emerald-500/20 text-emerald-400' : 'bg-slate-800 text-slate-400'
                }`}
              >
                <Icon className="w-5 h-5" aria-hidden="true" />
              </div>
              <div>
                <span className="text-[10px] uppercase tracking-wider block text-slate-400 font-bold">Step {step.id}</span>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="text-lg font-bold text-slate-100 flex items-center gap-2">
                <UserCheck className="w-5 h-5 text-indigo-400" aria-hidden="true" />
            ...
            <p className="text-xs text-slate-300 bg-slate-950 p-4 rounded-lg border border-slate-800">
              {impactResult?.impactSummary}
            </p>

            <div className="space-y-4 mt-4">
              {impactResult?.roleSpecificImpacts.map((item, idx) => (
                <div key={item.finding_id || idx} className="bg-slate-950 border border-slate-800 rounded-xl p-4 space-y-3">
                  <div className="flex items-center justify-between">
                  ...
                  <p className="text-xs text-slate-200">{item.what_this_means_for_you}</p>

                  <div className="bg-slate-900 p-3 rounded-lg border border-slate-800 space-y-1.5">
                    <span className="text-[11px] font-bold text-indigo-400 block">Practical Considerations:</span>
      ...
      {activeStep === 2 && (
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 space-y-4" role="tabpanel" aria-label="Step 2: Legal X-Ray Panel">
          <h3 className="text-lg font-bold text-slate-100 flex items-center gap-2">
            <Shield className="w-5 h-5 text-indigo-400" aria-hidden="true" />
            Legal X-Ray Findings Overview
          </h3>
          <p className="text-xs text-slate-400">Review structured findings classified by severity level and finding kind.</p>
          <LegalXRayDashboard
            overview={{
      ...
      {activeStep === 3 && (
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 space-y-4" role="tabpanel" aria-label="Step 3: Timeline & Milestones Panel">
          <h3 className="text-lg font-bold text-slate-100 flex items-center gap-2">
          ...
          <div className="space-y-3">
            <div className="bg-slate-950 p-4 rounded-xl border border-slate-800 flex items-start gap-3">
              <Calendar className="w-5 h-5 text-amber-400 shrink-0 mt-0.5" aria-hidden="true" />
              <div>
                <span className="text-xs font-bold text-slate-200 block">30-Day Resignation Notice Window</span>
                <span className="text-xs text-slate-400 block mt-0.5">Notice must be delivered 30 days prior to contract termination date.</span>
      ...
      {activeStep === 4 && (
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 space-y-4" role="tabpanel" aria-label="Step 4: Before You Sign Checklist Panel">
          <h3 className="text-lg font-bold text-slate-100 flex items-center gap-2">
          ...
          <div className="space-y-3">
            {impactResult?.questionsToAsk.map((q, idx) => (
              <div key={idx} className="bg-slate-950 p-3 rounded-lg border border-slate-800 flex items-start gap-2 text-xs text-slate-300">
      ...
      <div className="flex items-center justify-between border-t border-slate-800 pt-6">
        <button
          ...
          className="px-4 py-2 bg-slate-900 border border-slate-800 text-slate-300 text-xs font-semibold rounded-lg flex items-center gap-2 focus:outline-none focus:ring-2 focus:ring-indigo-500 disabled:opacity-40"
        >
        ...
        <button
          ...
          className="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-semibold rounded-lg flex items-center gap-2 focus:outline-none focus:ring-2 focus:ring-indigo-500 disabled:opacity-40"
        >
